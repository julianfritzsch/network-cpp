...
class Network {
 ...
 private:
  ...
  void lusolve(arma::mat& L, arma::mat& U, arma::mat& P,
               arma::vec& x);  // solve a linear system using a LU decomposition
  template <typename T>
  arma::vec bigradient(
      T& A, arma::vec b, arma::vec xk,
      double error = 1.0e-8);  // use bigradient decent to solve a linear system
